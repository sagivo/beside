...
`hooks.definitions`, or drop plugin hooks into `plugins/hook/` and list them in
`hooks.plugins`.
...
1. **Use a built-in renderer** — `list` or `json`.
...
widgets (`list`, `json`) cover the common shapes, so a
