...
```text
   ...
   apps & screen ──► [capture] ──► raw events ──► [storage] ──► frames + sessions + meetings
                                                   ...
                                                   [model]   ──► [index strategy]
                                                                       │
                                                                       ▼
                                                   Markdown wiki + chunks + memory tree
                                                                       │
                                                                       ▼
                                                                  [export]
                                                                       │
...
| Package | Role |
| --- | --- |
| `@beside/interfaces` | Pure type definitions for every plugin contract and data type (`RawEvent`, `Frame`, `MemoryChunk`, `MemoryLeaf`, `MemoryNode`, `IStorage`, `ICapture`, `IModelAdapter`, `IIndexStrategy`, `IExport`, `IHookPlugin`). |
| `@beside/core` | Config loading + Zod schema, structured logger, scheduler, load guard, plugin loader, ID + path helpers. |
| `@beside/runtime` | The orchestrator. Wires plugins together, manages capture-hook execution, embedding workers, memory-tree refreshes, OCR/audio workers, session/meeting/day-event derivation, and the storage vacuum. |
...
- **`MemoryLeaf` / `MemoryNode`** — the persistent memory tree above chunks.
  Leaves are admitted, evidence-backed memories; nodes roll leaves up by
  source, topic, day, or global scope.
...
7. The **embedding worker** keeps frame and memory-chunk embeddings up to date,
   then rebuilds the memory leaf/node tree.
8. **Export plugins** see every page write and memory update, then mirror them
   to disk (`markdown`) or expose them to outside agents (`mcp`).
...
interface IStorage {
  ...
  // …plus sessions, meetings, day events, memory chunks/leaves/nodes/jobs,
  // hook records, vacuum
}
...
│   ├── notes/…                  # optional user-authored memory notes
│   └── topics/…                 # hierarchical wiki pages
└── export/
    └── markdown/                # mirror of index/ plus _memory/tree.md
```
