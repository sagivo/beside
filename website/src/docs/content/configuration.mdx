...
runtime passes the whole entry to the plugin factory, so `path`, `host`, `port`,
`transport`, and `text_excerpt_chars` stay next to the `enabled` flag you use
from the desktop Settings screen.
