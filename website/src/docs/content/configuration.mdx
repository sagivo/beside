...
custom path via `beside <command> --config <path>`). The schema is defined and validated by
...
app:
  ...
  # session_id: my-install       # optional — set to pin the install id
```

`data_dir` is the root for the stock paths: raw events, assets, the index,
exports, and the SQLite database. Move it onto an external drive by changing
this one line, or set `BESIDE_DATA_DIR` to re-root the defaults without editing
the file. Any path you explicitly customize in YAML is left alone.
...
capture:
  ...
  audio:
    ...
    live_recording:
      enabled: true
      activation: other_process_input        # starts only when another process uses audio input
  ...
  # jpeg_quality: 70              # optional override for jpeg
  excluded_apps:
    - 1Password
    - Bitwarden
    - Keychain Access
  ...
  multi_screen: false
  # screens: [0, 1]              # optional display whitelist
...
This repository ships `ollama` and `openai` model plugins. The `claude` block
is preserved in the schema and desktop Settings UI for custom/future adapters;
setting `plugin: claude` only works once a matching `plugins/model/claude`
drop-in exists.
...
Export plugin settings live inline on each `export.plugins[]` entry. The
runtime passes the whole entry to the plugin factory, so `path`, `profile`,
`host`, `port`, `transport`, and `text_excerpt_chars` stay next to the
`enabled` flag you use from the desktop Settings screen. Markdown profiles
choose the link style: Obsidian/Logseq preserve wikilinks, while `portable`
writes relative Markdown links for Notion import, GitHub, editors, and static
sites.
...
| --- | --- |
| **Strict privacy / regulated team** | `capture.plugin: node`, narrow `excluded_apps`, set `excluded_url_patterns`, keep `index.model.plugin: ollama`, set the MCP export entry to `enabled: false` until ready, lower `storage.local.retention_days`. |
