...
- Supports optional multi-display capture with `multi_screen`, `screens`, and
...
- **Live audio recording** with chunked m4a output, sample-rate / channel
  ...
  backends.
- **Input-aware activation** — `other_process_input` starts live recording only
  while another process is already using audio input (so silent meetings don’t
  fill your disk).
- **Multi-screen** support via `multi_screen: true`, optional `screens: [0, 1]`,
  and `capture_mode: active | all`.
